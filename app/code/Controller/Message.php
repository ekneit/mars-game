...
class Message extends Controller
{
    ...
    public function index()
    {
        ...
        $allRecivedMessages = MessageModule::getAllrecevMessages($userSession->getAuthUserId());
        $recivedMessages = $this->getAllMessages($allRecivedMessages);

        $allSendMessages = MessageModule::getAllsendMessages($userSession->getAuthUserId());
    ...
    public function getAllMessages($messages)
    {
        ...
        foreach ($messages as $key => $message)
        {
            if ($message['receiver_id']) {
                $data[$key]['sender_email'] = $user->load($message['receiver_id'])->getEmail();
            }
    ...
    public function read($id)
    {
       ...
       if(!empty($getMessage)) {
           if($this->userSession->getAuthUserId() === $getMessage->getReceiverId()
               || $this->userSession->getAuthUserId() === $getMessage->getSenderId()){
